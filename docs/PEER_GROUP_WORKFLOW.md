...
- **`content_mode="summary"` (기본)**: 스냅샷에 1회 생성 후 고정 저장된 개요 요약(3~4문장) + 매출실적 표 발췌를 반환합니다 — 종목당 ~1KB로 경량이고, 저장된 요약을 그대로 읽으므로 재현 가능합니다. 원문(절단본)이 필요하면 `content_mode="full"` (종목당 ~5KB, `max_section_chars` 적용).
